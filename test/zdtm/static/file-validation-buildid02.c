...
const char *test_doc	= "File validation test for build-id with 2 PT_NOTE header in a 32 bit ELF file (Should fail during restore)";
...
int main(int argc, char **argv)
{
	...
	Elf32_Phdr program_header, program_header_bid;
	Elf32_Nhdr note_header, note_header_bid;

	int fd, name_size, desc_size, build_id_size, i, tmp = '0';
	char name[] = "GNU";
	char desc[] = "SHORT_UNIMPORTANT_STRING";
	...
	memset(&program_header_bid, 0, sizeof(program_header_bid));
	memset(&note_header, 0, sizeof(note_header));
	memset(&note_header_bid, 0, sizeof(note_header_bid));

	name_size = ALIGN(sizeof(name), 4);
	desc_size = ALIGN(sizeof(desc), 4);
	...
	program_header.p_offset			= sizeof(file_header) + 2*sizeof(program_header);
	program_header.p_vaddr			= sizeof(file_header) + 2*sizeof(program_header);
	program_header.p_paddr			= 0;
	program_header.p_filesz			= sizeof(note_header) + name_size + desc_size;
	program_header.p_memsz			= sizeof(note_header) + name_size + desc_size;
	program_header.p_align			= 1;

	program_header_bid.p_type		= PT_NOTE;
	program_header_bid.p_flags		= PF_R | PF_W;
	program_header_bid.p_offset		= sizeof(file_header) + 2*sizeof(program_header) +
							sizeof(note_header) + name_size +
							desc_size;
	program_header_bid.p_vaddr		= sizeof(file_header) + 2*sizeof(program_header) +
							sizeof(note_header) + name_size +
							desc_size;
	program_header_bid.p_paddr		= 0;
	program_header_bid.p_filesz		= sizeof(note_header_bid) + name_size +
							build_id_size;
	program_header_bid.p_memsz		= sizeof(note_header_bid) + name_size +
							build_id_size;
	program_header_bid.p_align		= 1;
	...
	note_header_bid.n_type			= NT_GNU_BUILD_ID;
	...
	if (write(fd, &file_header, sizeof(file_header)) != sizeof(file_header) ||
		...
		write(fd, &program_header_bid, sizeof(program_header_bid)) != sizeof(program_header_bid) ||
	...
	if (write(fd, desc, sizeof(desc)) != sizeof(desc))
		goto write_err;
	for (i = 0; i < desc_size-sizeof(desc); i++)
		if (write(fd, &tmp, 1) != 1)
			goto write_err;

	if (write(fd, &note_header_bid, sizeof(note_header_bid)) != sizeof(note_header_bid))
		goto write_err;
