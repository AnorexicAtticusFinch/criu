				...
				unsigned char **build_id)
{
	 ...
	 * file was mapped.
	 */
	file_header_end = min_t(size_t, fd_status->st_size, 1048576) + (size_t) file_header;
	...
	*build_id = (unsigned char *) xmalloc(size);
	...
	munmap(file_header, min_t(size_t, fd_status->st_size, 1048576));
	return size;
				...
				unsigned char **build_id)
{
	 ...
	 * file is mapped.
	 */
	start_addr = (char *) mmap(0, min_t(size_t, fd_status->st_size, 1048576),
	...
	munmap(start_addr, min_t(size_t, fd_status->st_size, 1048576));
	return -1;
}
